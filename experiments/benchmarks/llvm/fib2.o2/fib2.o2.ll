...
entry:
  ...
  br i1 %cmp5, label %return, label %if.else.preheader, !dbg !29

if.else.preheader:                                ; preds = %entry
  br label %if.else, !dbg !30

if.else:                                          ; preds = %if.else.preheader, %if.else
  %n.tr7 = phi i64 [ %sub1, %if.else ], [ %n, %if.else.preheader ]
  %accumulator.tr6 = phi i64 [ %add, %if.else ], [ 1, %if.else.preheader ]
  ...
  br i1 %cmp, label %return.loopexit, label %if.else, !dbg !29

return.loopexit:                                  ; preds = %if.else
  %add.lcssa = phi i64 [ %add, %if.else ]
  br label %return

return:                                           ; preds = %return.loopexit, %entry
  %accumulator.tr.lcssa = phi i64 [ 1, %entry ], [ %add.lcssa, %return.loopexit ]
